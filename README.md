...
- **Local deploy wrapper** (`scripts/deploy-local.sh`) for production deploys
...
```
┌─────────────────┐     ┌──────────────────┐     ┌─────────────────┐
│  WordPress      │────▶│  Local Builder   │────▶│  Cloudflare     │
│  (private/local)│     │  (Build & Deploy)│     │  Pages          │
└─────────────────┘     └──────────────────┘     └─────────────────┘
```
...
- GitHub account (for Giscus and optional baked comments fetch)
...
Local deployment uses the same Cloudflare credentials you entered in setup.sh.

Set these variables in your local shell or `.env` before deploying:
...
- `WP_API_URL` - Your WordPress REST API URL (for local/private WordPress, use the reachable local network hostname)
- `GITHUB_TOKEN` - GitHub Personal Access Token (for comments fetch when needed)
...
This is the primary production deployment path for private/local WordPress setups.

GitHub-hosted runners are not the default deploy path here because they cannot reliably reach private/local `WP_API_URL` endpoints without extra network plumbing.
...
| `GITHUB_TOKEN` | GitHub PAT for comments fetch | [Create here](https://github.com/settings/tokens) |
...
4. Trigger a new local deploy run
...
Note: `hugo-site/content/` is generated by sync/fetch scripts and is ignored in git.
...
1. Run `./scripts/deploy-local.sh` and check the first failing step
2. Verify required env vars are set (`WP_API_URL`, `CLOUDFLARE_API_TOKEN`, `CLOUDFLARE_ACCOUNT_ID`, `CLOUDFLARE_PAGES_PROJECT`)
3. Test Hugo directly: `docker compose exec builder hugo -s /app/hugo-site --minify`
...
wp-hugo-static automatically resizes, watermarks, and converts images to WebP. See [docs/image-optimization.md](docs/image-optimization.md) for current behavior and customization options.
