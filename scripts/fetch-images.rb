...
def optimize_images(image_paths)
  ...
  begin
    optimizer = ImageOptim.new(pngcrush: false, pngout: false, advpng: false, oxipng: false, jhead: false, jpegtran: false, gifsicle: false, svgo: false)
...
def process_image(path)
  ...
  return path if skip_svg?(filename)
  return path if animated_gif?(filename)
...
def fetch_featured_image(media_id, post_slug)
  ...
  wp_api_url = Sync::Config.wp_api_url
...
def update_content_image_paths(content, mapping)
  mapping.each { |original_url, new_path| content = content.gsub(original_url, new_path) }
  content
end
...
def generate_watermark(text, output_path, size: 100)
  ...
  height = 30
  safe_text = text.gsub("'", "'\\''")
  cmd = "convert -size #{width}x#{height} xc:transparent -font '#{font_name}' -pointsize 14 -fill white -gravity center -annotate +0+0 '#{safe_text}' #{output_path}"
  result = system(cmd)
